...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            'type',
            sa.Enum(
                'started', 'finished', 'interrupted', 'in_progress', 'canceled', name='actions_type', native_enum=False
            ),
            nullable=False,
        ),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_index(
        op.f('ix_frontiers_characters_account_id'), 'characters', ['account_id'], unique=False, schema='frontiers'
    )
    op.create_index(
