...
async def test_create_db_pool_creates_fixed_warm_pool(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr('app.lifespans.db.asyncpg.create_pool', create_pool)
    config = DbConfig(
        _env_file=None,
        password='secret',
        pool_size=7,
...
async def test_create_db_pool_closes_pool_when_health_check_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr('app.lifespans.db.asyncpg.create_pool', AsyncMock(return_value=pool))
